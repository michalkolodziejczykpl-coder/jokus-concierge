...
- ✅ Supabase migrations: **29 versioned files** in `supabase/migrations/` (initial schema, RLS, phase-1 fixes, slot finder/hold, sprint-4 order lifecycle, marketplace, grocery catalog, jokusor onboarding, seeds, billing v2 `20260706000001`, billing v3 + gastro `20260721000001`, SMS send log `20260722000001`…), applied to the live dev project. RLS is enabled on all tables.
- ✅ **Anti-bot / SMS-budget hardening (2026-07-22),** after ~6 bot signups (gmail dot-aliases, none ever confirmed e-mail → zero cost; deleted via Auth Admin API): Cloudflare Turnstile on register/password-login/password-reset (`TurnstileWidget`, soft-fail without `NEXT_PUBLIC_TURNSTILE_SITE_KEY`; Supabase "CAPTCHA protection" switch is ON in the dashboard — flipping it off does not break the forms, they just stop requiring tokens), Supabase dashboard rate limits tightened, explicit `email_confirmed_at` guard in /api/register/profile + confirm-phone. SMS: `sms_send_log` table + daily budgets enforced in the `send-sms` edge function (defaults 100/day global, 5/day per phone; owner-tunable WITHOUT redeploy via function secrets `SMS_DAILY_GLOBAL_LIMIT` / `SMS_DAILY_PHONE_LIMIT` / `SMS_ALERT_PHONE`), owner SMS alerts at 80%/100% of the global budget (max once per threshold per Warsaw day — DB-guaranteed by partial unique index `idx_sms_log_alert_once_per_day`; claim-row-then-send), usage counter + rejection banner on the admin /panel. Fail-open when the log table is unreachable (budget protects money, not security).
...
- ⚠️ **Payments are mocked.** `mock_pay_order` SQL fn + `/api/payments/mock-pay`, gated by `MOCK_PAYMENT_ALLOWLIST` (fail-closed in production). The Przelewy24 webhook is still a TODO stub — no real money moves yet. **⚠️ TEMPORARY P24-VERIFICATION BYPASS ACTIVE (2026-07-22): `isMockPaymentAllowed` returns `true` for every signed-in user** (covers BOTH callers: mock-pay and tips). The allowlist kept rejecting the test account on production for an undiagnosed reason and the verification blocked the pilot; owner accepted the risk (no traffic, mock money, session still required). Strict gate preserved as `strictMockPaymentGate` — one-line restore.
- **🧹 CLEANUP CHECKLIST — after the P24 verification (in this order):**
  1. Restore the strict gate: `isMockPaymentAllowed` → `return strictMockPaymentGate(email)` in `lib/payments/mockGate.ts`.
  2. Remove `MOCK_PAYMENT_ALLOWLIST` from Vercel env (Production + Preview) + redeploy — back to fail-closed. (Also finish diagnosing WHY the allowlist rejected `jokustest@gmail.com` — the strict gate now logs `[mockGate] denied` diagnostics with entry lengths.)
  3. Change the `jokustest@gmail.com` password (shared in plaintext with P24 testers and present in a chat transcript) — `/reset-haslo` or Auth Admin API.
  4. Delete test accounts registered by P24 testers (Auth Admin API — cascades to public.users, same as the bot cleanup).
  5. Re-enable CAPTCHA in Supabase (Auth → Attack Protection) — turned OFF 2026-07-22 after production rejected solved Turnstile tokens. BEFORE the switch: finish the paused investigation (suspects: Secret Key from a different widget than Site Key `0x4AAAAAAD…`, hostname coverage, token expiry — `refresh-expired: 'auto'` typed but not wired). Test password login immediately after flipping it.
...
- ❌ **PWA icons missing** (known gap, noted 2026-07-22, deliberately unfixed for now): `public/manifest.json` declares `/icons/icon-192.png`, `/icons/icon-512.png` and `/icons/icon-maskable.png` (512, purpose maskable), but `public/icons/` contains only a `.gitkeep` — every page load logs a 404 for icon-192 (browser fetches it via the manifest) and the Android install prompt has no icon. Fix = drop three PNGs with exactly these names into `public/icons/`.
...
- **Every "read active/public" RLS policy must be reviewed at the COLUMN level, not just the row level.** RLS grants row access to the WHOLE row — it cannot hide columns. A `USING (is_active)` / `USING (role='jokusor')` / `USING (status='active')` policy therefore exposes EVERY column of the matched rows to whoever the policy admits (often `anon`). Public/other-user reads go through a SECURITY DEFINER view that selects only safe columns; never point the UI at a base table for another person's row.
- **RLS INCIDENT (RODO), policies dated 16.05.2026, detected 24.07, patched 24.07 (`20260724000001_public_safe_views`):** the public read policies `users_read_public_profile`, `jokusors_read_active`, `listings_read_active` exposed full rows to ANONYMOUS PostgREST callers — jokusors' `nip`/`bank_account`/`background_check_url` (criminal-record scan path)/`service_postal_codes`, every jokusor's `email`/`phone`, and each active listing's `pickup_address` (seller's home address, no `moderation_status` filter). Fixed with two public-safe views (`public_jokusor_profiles`, `public_listings`) + dropping the three policies; base `marketplace_listings` read narrowed to seller/admin; `pickup_address` reaches the buyer only via their order (SECURITY DEFINER purchase fn). Resident-to-resident was never affected. The buy/slot paths use SECURITY DEFINER fns so they were untouched.
